...
const documents: Document[] = [
  { id: "mock-1", name: "FY26 Product Roadmap", collection: "Product", type: "PPTX", updated: "Today, 10:42 AM", owner: "Maya Chen", status: "Ready", size: "4.2 MB" },
  { id: "mock-2", name: "Employee Handbook", collection: "People & policies", type: "DOCX", updated: "Yesterday", owner: "Priya Shah", status: "Ready", size: "1.8 MB" },
  { id: "mock-3", name: "Q2 Sales Report", collection: "Sales operations", type: "PDF", updated: "Aug 8", owner: "Alex Morgan", status: "Ready", size: "2.6 MB" },
  { id: "mock-4", name: "Security Incident Playbook", collection: "Engineering", type: "PDF", updated: "Aug 7", owner: "David Kim", status: "Processing", size: "932 KB" },
  { id: "mock-5", name: "Customer Discovery Notes", collection: "Product", type: "DOCX", updated: "Aug 5", owner: "Maya Chen", status: "Ready", size: "688 KB" },
  { id: "mock-6", name: "FY26 Hiring Plan", collection: "People & policies", type: "XLSX", updated: "Aug 1", owner: "Priya Shah", status: "Ready", size: "1.1 MB" },
];

const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000";

function toDocument(document: {
  id: string; name: string; collection: string; content_type: string; size_bytes: number; owner: string; status: string; updated_at: string;
}): Document {
  const extension = document.name.split(".").pop()?.toUpperCase() ?? "FILE";
  return {
    id: document.id, name: document.name, collection: document.collection, type: extension as Document["type"],
    updated: new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(document.updated_at)),
    owner: document.owner, status: document.status === "ready" ? "Ready" : "Processing",
...
export function DocumentKnowledgeUI({ initialView }: { initialView: "knowledge" | "documents" }) {
  ...
  useEffect(() => {
    fetch(`${apiUrl}/api/v1/documents`)
      .then((response) => response.ok ? response.json() : Promise.reject())
      .then((data) => { if (data.length) setDocumentRows(data.map(toDocument)); })
      .catch(() => undefined);
  }, []);
  ...
  async function uploadFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/v1/documents`, { method: "POST", body: formData });
  ...
  return <section className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      ...
      <button onClick={() => fileInput.current?.click()} className="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-indigo-700">Upload document</button>
    </header>
    ...
    {uploadError && <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">{uploadError}</div>}
    <input ref={fileInput} onChange={uploadFile} accept=".pdf,.docx,.pptx,.xlsx" className="hidden" type="file" />
